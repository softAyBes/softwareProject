	 ...
	 int index=0;
		...
		for(int i=0;i<customers2.size();i++)
		{
			...
			for(int j=0;j<customers2.get(i).getProductsForCusomer().size();j++)
			{
				...
				//customers2.get(i).getProductsForCusomer().get(j).setPrice(money_after_dis);
